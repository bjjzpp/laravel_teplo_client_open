						...
						<article class="page type-page status-publish has-post-thumbnail hentry">
							<header class="entry-header"><h2 class="entry-title">Редактируем</h2></header>
								<div class="entry-content">
                                    <form name="feedback" method="post" action="{{ route('admin.home.pay.update', ['id' => $Pay->id]) }}" class="form-horizontal" >
                                    @csrf
                                    <div class="form-group row">
                                    <label for="label_id" id="label_id" class="col-md-4 col-form-label text-md-right">№: {{$Pay->id}}</label>
                                    </div>
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-6">
                                            <select name="id_spr_bank" id="id_spr_bank" class="form-control form-control-sm">
                                                @foreach($SprBank as $site_tg)
                                                    <option value="{{ $site_tg->id }}"
                                                        @if($Pay->id_spr_bank == $site_tg->id)
                                                            selected
                                                        @endif
                                                        >{{ $site_tg->mini_bank }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-6">
                                            <input id="title" type="text" class="form-control{{ $errors->has('title') ? ' is-invalid' : '' }}" name="title" value="{{ $Pay->title }}" size="200px">
                                            @if ($errors->has('title'))
                                    ...
                                    <div class="form-group row">
                                        ...
                                        <div class="col-md-6">
                                            <textarea id="full_text" class=" form-control{{ $errors->has('full_text') ? ' is-invalid' : '' }}" name="full_text" rows="7" cols="180" class="form-control">{!! $Pay->full_text !!}</textarea>
