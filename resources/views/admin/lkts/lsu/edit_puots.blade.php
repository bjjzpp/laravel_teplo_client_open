			...
			<div id="content" class="site-content"><div class="container"><div class="inner-wrapper">
				<div id="primary" class="content-area">
					<main id="main" class="site-main" role="main">
                            <a href="{{ route('admin.lkts.lsu.edit', ['id' => $lkts_ls_puot->id_lkts_ls]) }}">Назад</a><br /><br />
						<article id="post-33" class="post-33 page type-page status-publish has-post-thumbnail hentry">
                                <p class="text-center text-danger font-weight-light bg-warning"><strong>Внимание!<br />Перед отправкой данных, проверьте правильность заполнения данных по показаниям прибора учета!</strong></p>
                                <header class="entry-header"><h2 class="entry-title">Адрес: {{ $lkts->rco_maps->name }}, кв. {{ $lkts->office }} / {{ $lkts_ls_puot->title }}</h2></header>
                                    <div class="entry-content">
                                    @if($lkts_ls_puot->flag_write == 0)
                                    <form name="feedback" method="post" action="{{ route('admin.lkts.lsu.send_puots') }}" class="form-horizontal">
                                    @csrf
                                    @endif
                                    ...
                                    @endforeach
                                    <div class="text-right">@if($lkts_ls_puot->flag_write == 0)<a href="{{ route('admin.lkts.lsu.edit_puots_addp', ['id' => $id ]) }}" class="pull-right btn btn-outline-dark btn-sm">Добавить показания</a>@endif</div><br />
